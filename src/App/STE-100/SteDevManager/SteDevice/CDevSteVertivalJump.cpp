#include "CDevSteVertivalJump.h"
#include "SteSubElement/CSubElementVerticalJump.h"

CDevSteVertivalJump::CDevSteVertivalJump(unsigned int iDevID, ICommunicate &communicate)
    :CDevHardwareBase(iDevID, communicate)
    , m_shDataLen(sizeof (CElementDataVerticalJump))
{
    m_shDevType = DEV_TYPE_VJUMP;

    initSubElement<CSubElementVerticalJump>(SubElementPropertye::ELEMENT_NUM_VJump);
    initDevInfo();
}

CDevSteVertivalJump::~CDevSteVertivalJump()
{
}
...
/*************************************************
  ...
  <时    间>   2022-02-22
  **************************************************/
int CDevSteVertivalJump::parseSinglePackage(const unsigned int &iFrameNo, unsigned char *pData, int iDatalen)
{
    ...
    static CElementDataVerticalJump elementData;
    //获取状态位
