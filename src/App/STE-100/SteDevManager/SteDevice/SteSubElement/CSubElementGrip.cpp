...
#include "CSubElementGrip.h"
//握力使用肺活量的防作弊逻辑
#include "TestProcAntiCheating/CTestProcAntiCheatingVC.h"


CSubElementGrip::CSubElementGrip(unsigned short shUnitID,  unsigned int iElementID)
    :CSubElementBase(shUnitID, iElementID, createExpectObj(DEV_TYPE_GRIP))
{
    setCurTestItem(TEST_ITEM_GRIP);
    m_shDevType = DEV_TYPE_GRIP;
}

CSubElementGrip::~CSubElementGrip(){

}

void CSubElementGrip::setElementInfo(void *pData, unsigned short shLength)
{
    ...
    CElementDataGrip *pElementData = reinterpret_cast<CElementDataGrip *>(pData);
...
void CSubElementGrip::startTest()
{
    m_bStartFlag = true;
    m_objData.reset();
    clearErr();
...
void CSubElementGrip::createAntiCheating()
{
    deleteAntiCheating();
