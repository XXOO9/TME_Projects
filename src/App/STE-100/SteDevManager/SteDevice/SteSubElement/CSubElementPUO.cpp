...
#include "CSubElementPUO.h"
#include "TestProcAntiCheating/CTestProcAntiCheatingPullUp.h"

CSubElementPUO::CSubElementPUO(unsigned short shUnitID, unsigned int iElementID)
    :CSubElementBase(shUnitID, iElementID, createExpectObj(DEV_TYPE_PUO))
{
    setCurTestItem(TEST_ITEM_PUO);
    m_shDevType = DEV_TYPE_PUO;
}

CSubElementPUO::~CSubElementPUO()
{

}

void CSubElementPUO::setElementInfo(void *pData, unsigned short shLength)
{
    if(pData == nullptr || shLength == 0)
    {
        LOGERROR("CSubElementPUO::setElementInfo err");
        return;
    }

    CElementDataPUO *pElementData = reinterpret_cast<CElementDataPUO *>(pData);
    ...
    bool bSignalDataChange = pElementData->m_nData != m_objData.m_nData != 0;
...
void CSubElementPUO::createAntiCheating()
{
    deleteAntiCheating();
