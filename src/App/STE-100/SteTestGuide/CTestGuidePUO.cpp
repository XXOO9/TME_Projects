...
#include "SteDevManager/SteDevice/SteSubElement/CSubElementPUO.h"
#include "SteDevManager/CSteDeviceManager.h"
#include "CDevDataCfg.h"
using namespace TestTableCfg;

CTestGuidePUO::CTestGuidePUO()
    :CTestGuideBase ()
{
    m_shDevType = DEV_TYPE_PUO;
}

void CTestGuidePUO::setSteDevice(CDevHardwareBase *pSteDev)
{
    if(m_iDevID == 0)
    {
        m_vecTestItem.clear();
        m_vecTestItem.push_back(TEST_ITEM_PUO);
    }

    CTestGuideBase::setSteDevice(pSteDev);
}

void CTestGuidePUO::onDataChange(const unsigned short &shElementIndex, const unsigned char cDataStstus)
{
    ...
    CSubElementPUO *pElement = reinterpret_cast<CSubElementPUO *>(pSubElement);
    //肺活量数据
...
void CTestGuidePUO::saveDataToTestResult(CElementInfo &elementInfo, CSubElementBase *pElement)
{
    if(pElement == nullptr)
    {
    ...
    //记录本次成绩
    CSubElementPUO *pSubElement = reinterpret_cast<CSubElementPUO *>(pElement);
