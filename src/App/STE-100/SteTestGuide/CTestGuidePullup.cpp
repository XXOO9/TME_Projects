...
#include "SteDevManager/SteDevice/SteSubElement/CSubElementPullUp.h"

CTestGuidePullup::CTestGuidePullup()
    :CTestGuideBase ()
{
    m_shDevType = DEV_TYPE_PULL_UP;
}

void CTestGuidePullup::setSteDevice(CDevHardwareBase *pSteDev)
{
    if(m_iDevID == 0)
    {
        m_vecTestItem.clear();
        m_vecTestItem.push_back(TEST_ITEM_PULL_UP);
    }

    CTestGuideBase::setSteDevice(pSteDev);
}

void CTestGuidePullup::onDataChange(const unsigned short &shElementIndex, const unsigned char cDataStstus)
{
    ...
    CSubElementPullUp *pElement = reinterpret_cast<CSubElementPullUp *>(pSubElement);
    ...
    //肺活量数据
    //将数据设置到ui
    QString qsDisp = value2DispValue(pElement->getCurTestItem(), static_cast<float>(pElement->getDataPullup()));
    dispTestResult(qsDisp, *pElementInfo);
}

void CTestGuidePullup::saveDataToTestResult(CElementInfo &elementInfo, CSubElementBase *pElement)
{
    if(pElement == nullptr)
    {
    ...
    //记录本次成绩
    CSubElementPullUp *pSubElement = reinterpret_cast<CSubElementPullUp *>(pElement);
    QString qsValueDisp = value2DispValue(static_cast<short>(resultInfo.m_nItem), static_cast<float>(pSubElement->getDataPullup()));
    resultInfo.addTestResult(qsValueDisp.toStdString(), bCredible);

    //将数据设置到ui
    float fSbjData = static_cast<float>(pSubElement->getDataPullup());
