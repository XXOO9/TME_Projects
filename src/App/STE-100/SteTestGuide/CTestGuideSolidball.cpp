...
CTestGuideSolidball::CTestGuideSolidball()
    :CTestGuideBase ()
{
    m_shDevType = DEV_TYPE_SOLIDBALL;
}

void CTestGuideSolidball::setSteDevice(CDevHardwareBase *pSteDev)
{
    if(m_iDevID == 0)
    {
        m_vecTestItem.clear();
        m_vecTestItem.push_back(TEST_ITEM_SOLIDBALL);
    }

    CTestGuideBase::setSteDevice(pSteDev);
}

void CTestGuideSolidball::onDataChange(const unsigned short &shElementIndex, const unsigned char cDataStstus)
{
...
void CTestGuideSolidball::saveDataToTestResult(CElementInfo &elementInfo, CSubElementBase *pElement)
{
    if(pElement == nullptr)
    {
    ...
    else
    {
        ...
        {
            bCredible =  elementInfo.m_cHardwareDataStateCode == DEV_TEST_STATE_EOT_VALID;
        }
    }
