...
Item {
    ...
    property int curTestCount: 0        //当前测试项目计数
    ...
    property string curWeightVal: "0"
    property string curHeightVal: "0"
    ...
    Connections{
        ...
        onSignalCurrentBodyTestValue: {
            //如果数据没有变化,则直接退出
            InterAction.qmlLogout( "curWeight = " + weightVal + " curHeight = " + heightVal )
            if(curWeightVal === weightVal && curHeightVal === heightVal)
            {
                return
            }
            else
            {
                curWeightVal = weightVal
                curHeightVal = heightVal
            }

            if(curWeightVal * 1 && curHeightVal * 1)
            {
                //身高体重测量仪，有数据刷新时，就结束超时定时器
                timeoutTimer.stop()
            }

            doubleRectTestRet.weightVal = curWeightVal;
            doubleRectTestRet.heightVal = curHeightVal;

            InterAction.qmlLogout( "refresh qml view testW ret = ", curWeightVal , "-", curHeightVal);
        }
