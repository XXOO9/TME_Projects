...
Item {
    ...
    CusDoubleRectWithCountTime{
        ...
        state: "ready"
        shortCountTime: 40
        anchors{ left: parent.left; leftMargin: 170 * factor; top: parent.top; topMargin: 371 * factor }
        onStateChanged: {
            if( testPhaseDoubleRect.state === 'testing' ){
                testPhaseDoubleRect.startTimer()
            }

            if( testPhaseDoubleRect.state === 'finished' ){
                testPhaseDoubleRect.startTimer( false )
            }
        }
    }
    ...
    */
    ...
    Timer{
        ...
        onTriggered: {
            autoSwitchCount--
            console.log( "auto exit timer count time = " + autoSwitchCount )
