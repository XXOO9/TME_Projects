...
void CHttpAbstractNetWork::cleanMultiParaMap()
{
...
QByteArray CHttpAbstractNetWork::toJson(requestType type)
{
    ...
    case TOURISTRECORD:
        ...
        return jsonData;
    case UPDATEUSERBINDCARDNUMBER:
        jsonData = toUpdateUserBindCardNumber();
        return jsonData;
    case DISBINDUSERCARDNUMBER:
        jsonData = toDisBindUserCardNumber();
        return jsonData;
    case USERLOGIN:
        jsonData = toUserLogin();
        return jsonData;
    default:
        break;
    }
...
QByteArray CHttpAbstractNetWork::toUpdateUserBindCardNumber()
{
    QJsonObject jsObj;
    QString userIdVal = m_multiParaMap.value( BINDCARD_KEY_UPDATEUSERNUMBER_USERID );
    QString cardNoVal = m_multiParaMap.value( BINDCARD_KEY_UPDATEUSERNUMBER_CARDNO );
    QString cardTypeVal = m_multiParaMap.value( BINDCARD_KEY_UPDATEUSERNUMBER_CARDTYPE );

    jsObj.insert( BINDCARD_KEY_UPDATEUSERNUMBER_USERID, userIdVal );
    jsObj.insert( BINDCARD_KEY_UPDATEUSERNUMBER_CARDNO, cardNoVal );
    jsObj.insert( BINDCARD_KEY_UPDATEUSERNUMBER_CARDTYPE, cardTypeVal );
...
QByteArray CHttpAbstractNetWork::toDisBindUserCardNumber()
{
    QJsonObject jsObj;
    QString userId = m_multiParaMap.value( BINDCARD_KEY_DISBINDCARDNUMBER_USERID );
    jsObj.insert( BINDCARD_KEY_DISBINDCARDNUMBER_USERID, userId );
    QByteArray sendJson = jsonObjectToByteArray( jsObj );
    return sendJson;
}

QByteArray CHttpAbstractNetWork::toUserLogin()
{
    QJsonObject jsObj;
    QString account = m_multiParaMap.value( LOGIN_ACCOUNT_KEY );
    QString passwd = m_multiParaMap.value( LOGIN_PASSWD_KEY );

    jsObj.insert( LOGIN_ACCOUNT_KEY, account );
    jsObj.insert( LOGIN_PASSWD_KEY, passwd );
